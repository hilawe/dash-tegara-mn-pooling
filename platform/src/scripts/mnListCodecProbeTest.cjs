 ...
 * (a soundness review). That is exactly the outcome a synthetic round trip would have hidden, which is why
 * one was refused.
 *
...
// ---- CANARY 1: the round trip through raw bytes is BROKEN on this real capture (a soundness review) ----
{
  let threw = null;
  try {
  ...
  ok("CANARY: serialize-then-parse of the real capture still fails on nType (a soundness review)",
...
// (a soundness review). These checks pin what the bytes are, so the parser written against them in step 3 has
// a fixed target.
// ---------------------------------------------------------------------------
{
  ...
  // CANARY: the library cannot parse the real wire form either (a soundness review). This fails if
  // the library gains support, which is the signal to re-read a soundness-review finding before choosing a parser.
  {
    ...
    ok("CANARY: the library still cannot parse the P2P wire form (a soundness review)", threw !== null);
  }
